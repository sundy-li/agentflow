...
Reuse the existing worktree and branch if they already exist for this issue. Verify the branch contains the implementation commit, push it to fork repository '{{repo_forked}}', and create or update the pull request from fork '{{repo_forked}}' to upstream '{{repo_full_name}}' targeting '{{default_branch}}'.
